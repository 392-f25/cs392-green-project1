...
type LayoutContextType = {
  setActiveSectionAndChat: (section: 'buy' | 'post' | 'my-listings' | 'my-chats', chatId?: string) => void;
  activeSection: 'buy' | 'post' | 'my-listings' | 'my-chats';
...
const Layout = ({ user }: Props) => {
  ...
  const [highlightedChatId, setHighlightedChatId] = useState<string | undefined>(undefined);

  const setActiveSectionAndChat = (section: 'buy' | 'post' | 'my-listings' | 'my-chats', chatId?: string) => {
    setActiveSection(section);
    if (section === 'my-chats' && chatId) {
  ...
  return (
    <LayoutContext.Provider value={contextValue}>
      <div className="min-h-screen bg-slate-50">
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex h-16 items-center justify-between">
              <div className="flex items-center gap-8">
                <button className="flex-shrink-0 bg-transparent border-none" onClick={() => setActiveSection('buy')}>
        ...
        <main>
          {activeSection === 'buy' && <BuyTickets user={user} />}
          {activeSection === 'post' && <PostListing user={user} />}
          {activeSection === 'my-listings' && <MyListings user={user} />}
          {activeSection === 'my-chats' && <MyChats user={user} highlightedChatId={highlightedChatId} />}
        </main>
